...
extension View {
    ...
    func clearFullScreenCover<Content: View>(isPresented: Binding<Bool>, style: UIBlurEffect.Style? = nil, onDismiss: (()->Void)? = nil, @ViewBuilder content: @escaping ()->Content) -> some View {
        self
            .fullScreenCover(isPresented: isPresented, onDismiss: onDismiss, content: {
                ...
                .background(ClearFullScreenCoverView(style: style)
                    .ignoresSafeArea()
                    .onTapGesture {
                        DispatchQueue.main.async {
