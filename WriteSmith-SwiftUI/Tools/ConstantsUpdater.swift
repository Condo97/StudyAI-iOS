...
class ConstantsUpdater: ObservableObject {
    ...
    @AppStorage("liveSpeechVoice") var liveSpeechVoice: Session.Voice = .alloy
    
    @Published var liveSpeechSpeed: Double
//    @Published var liveSpeechVoice: SpeechVoices
    
    private var cancellables: Set<AnyCancellable> = []
    
    init() {
        liveSpeechSpeed = ConstantsUpdater.liveSpeechSpeed
//        liveSpeechVoice = ConstantsUpdater.liveSpeechVoice
        
        $liveSpeechSpeed.sink(receiveValue: { speechSpeed in
            ConstantsUpdater.liveSpeechSpeed = speechSpeed
        })
        .store(in: &cancellables)
//        $liveSpeechVoice.sink(receiveValue: { speechVoice in
//            ConstantsUpdater.liveSpeechVoice = speechVoice
//        })
//        .store(in: &cancellables)
    }
...
extension ConstantsUpdater {
    ...
    static var liveSpeechSilenceDuration: Double {
        get {
            UserDefaults.standard.double(forKey: Constants.UserDefaults.liveSpeechSilenceDuration)
        }
        set {
            UserDefaults.standard.set(newValue, forKey: Constants.UserDefaults.liveSpeechSilenceDuration)
        }
    }
    
    static var liveSpeechSpeed: Double {
        get {
            let speed = UserDefaults.standard.double(forKey: Constants.UserDefaults.liveSpeechSpeed)
            return speed == 0 ? 1.0 : speed
        }
        set {
            UserDefaults.standard.set(newValue, forKey: Constants.UserDefaults.liveSpeechSpeed)
        }
    }
    
//    static var liveSpeechVoice: SpeechVoices {
//        get {
//            if let speechVoiceString = UserDefaults.standard.string(forKey: Constants.UserDefaults.liveSpeechVoice),
//               let speechVoice = SpeechVoices(rawValue: speechVoiceString) {
//                return speechVoice
//            }
//            
//            return SpeechVoices.allCases.first ?? .nova
//        }
//        set {
//            UserDefaults.standard.set(newValue.rawValue, forKey: Constants.UserDefaults.liveSpeechVoice)
//        }
//    }
