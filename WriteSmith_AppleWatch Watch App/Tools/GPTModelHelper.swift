...
class GPTModelHelper {
    ...
    static var currentChatModel: GPTModels {
        get {
            ...
            if UserDefaults.standard.string(forKey: Constants.UserDefaults.userDefaultStoredChatGPTModel) == nil || GPTModels.from(id: UserDefaults.standard.string(forKey: Constants.UserDefaults.userDefaultStoredChatGPTModel) ?? "") == nil {
                return defaultModel
            }
            
            // Get the model and check for premium before returning
            let model = GPTModels.from(id: UserDefaults.standard.string(forKey: Constants.UserDefaults.userDefaultStoredChatGPTModel) ?? "")
